...
import { StyleSheet, Text, View, Linking, Alert, KeyboardAvoidingView } from 'react-native'
...
const CreateEmployee = ({ navigation, route }) => {
    ...
    const pickFromGallery = async () => {
        ...
        if (granted) {
            ...
            if (!data.cancelled) {
                let newFile = {
                    uri: data.uri, type: `test/${data.uri.split(".")[1]}`,
    ...
    const handleUpload = (image) => {
        ...
        data.append('cloud_name', 'raghu106')

        fetch("https://api.cloudinary.com/v1_1/raghu106/image/upload", {
            method: "post",
            body: data
        }).then(res => res.json())
            .then(data => {
                ...
                Alert.alert("Something went  while uploading")
            })
    }
